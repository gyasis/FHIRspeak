...
class ClusterTreeBuilder(TreeBuilder):
    ...
    def build_from_leaf_nodes(self, leaf_nodes: List[Node]) -> Tree:
        """
        Builds the tree from a list of leaf nodes.
        
        Args:
            leaf_nodes (List[Node]): A list of leaf nodes to build the tree from.
        
        Returns:
            Tree: The constructed tree.
        """
        # Ensure leaf_nodes is a dictionary
        layer_to_nodes = {0: leaf_nodes}
        all_tree_nodes = {node.index: node for node in leaf_nodes}
        root_nodes = self.construct_tree(all_tree_nodes, all_tree_nodes, layer_to_nodes)

        tree = Tree(all_tree_nodes, root_nodes, {node.index: node for node in leaf_nodes}, self.num_layers, layer_to_nodes)
        return tree
