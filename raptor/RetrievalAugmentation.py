...
from .utils import split_text  # Ensure this import is here
...
class RetrievalAugmentation:
    ...
    def add_to_existing(self, docs):
        """
        Adds documents to the existing tree without overwriting it.
        ...
        if self.tree is None:
            raise ValueError(
                "There is no existing tree. Use 'add_documents' to create a new tree."
            )

        # Extract the leaf nodes from the existing tree
        existing_leaf_nodes = list(self.tree.leaf_nodes.values())

        # Split the new document into chunks and create new leaf nodes
        new_chunks = split_text(docs, self.tree_builder.tokenizer, self.tree_builder.max_tokens)
        new_leaf_nodes = self.tree_builder.multithreaded_create_leaf_nodes(new_chunks)

        # Add the new leaf nodes to the existing leaf nodes
        all_leaf_nodes = existing_leaf_nodes + list(new_leaf_nodes.values())

        # Rebuild the tree with the combined leaf nodes
        self.tree = self.tree_builder.build_from_leaf_nodes(all_leaf_nodes)
        self.retriever = TreeRetriever(self.tree_retriever_config, self.tree)

        logging.info("Successfully added new documents to the existing tree.")
